...
//FUNCIONA CONDICION MEDICION CADA 30MIN (RUDIMENTARIO)

#include "sapi.h"        // <= Inclusion de la Biblioteca sAPI

#define TIEMPO_30MIN     1800000  // ms (2 min = 120000 ms ) (30 min = 1800000 ms )
/*=====================[Logos LCD]=================================*/
...
/*=====================[Variables Multiples]=================================*/
   const float UMBRAL = 74; //Valor limitante de estres por calor para ganado lechero
...
void CONFIG_PINS(void){
        ...
        // Inicializar GPIOs
         ...
         gpioInit( GPIO6, GPIO_INPUT ); //selector AUTOMATICO-MANUAL


         // Configuracion de pines de salida para Leds de la EDU-CIAA-NXP
         gpioInit( GPIO3, GPIO_OUTPUT ); //FAN1
         gpioInit( GPIO5, GPIO_OUTPUT ); //FAN2
         gpioInit( GPIO7, GPIO_OUTPUT ); //FAN3
         gpioInit( GPIO8, GPIO_OUTPUT ); //FAN4
...
void ITH_INI(void){
       ...
       gpioWrite(GPIO3, HIGH);           //Desactiva salida reley FAN 1
       gpioWrite(GPIO5, HIGH);           //Desactiva salida reley FAN 2
       gpioWrite(GPIO7, HIGH);           //Desactiva salida reley FAN 3
       gpioWrite(GPIO8, HIGH);           //Desactiva salida reley FAN 4
       INICIADO_LCD();

        if(gpioRead(GPIO6)==LOW){
          lcdClear();                    // Borrar la pantalla
          ESTADO=MIDIENDO;
         }
...
void ITH_ENCENDIDO(void){
       ...
       gpioWrite(GPIO3, LOW);           //Desactiva salida reley FAN 1
       gpioWrite(GPIO5, LOW);           //Desactiva salida reley FAN 2
       gpioWrite(GPIO7, LOW);           //Desactiva salida reley FAN 3
       gpioWrite(GPIO8, LOW);           //Desactiva salida reley FAN 4
       PLOT_LCD(temperature, humidity, ITH);
       MONITOREO_TH(temperature, humidity);
       COUNT_TIME ++;
       printf("% TIMER: %d \r\n",COUNT_TIME);
