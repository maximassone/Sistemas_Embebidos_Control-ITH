...
int main( void )
{
   ...
   float ITH;
   int INI =0;                              //Señal de BOMBA OFF o BOMBA ON (perilla MANUAL a AUTO)
   unsigned long  tiempo_med;                        //Variable para tiempo de espera medicion
   const unsigned long  tiempo_medicion=500;     //Tiempo de espera entre mediciones (30min=1800000)
   ...
   char ONString[] = "ON";
   char OFFString[] = "OFF";
   int UMBRAL = 72;// 68; //Valor Umbral
   tiempo_med= 0; //Valor inicial timer
   ...
   while( TRUE ) {
      ...
      if( dht11Read(&humidity, &temperature)) {
         ...
         printf( "Humedad: %.2f  %.\r\n", humidity );
         if(INI ==0 || tiempo_med > tiempo_medicion){
